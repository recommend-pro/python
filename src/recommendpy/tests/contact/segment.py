...
class ContactSegmentTestCase(BaseTestCase):
    def setUp(self):
    ...
    def runTest(self):  # NOQA
        ...
        self.assertEqual(
            self.api_namespace.attach(
                identifier=self._id, field='email', identifiers=test_emails
            ),
            True
        )
...
def suite(api):
    namespace = api.contact.segment
    suite = get_suite(namespace)
    suite.addTest(ContactSegmentTestCase(namespace))
    return suite
